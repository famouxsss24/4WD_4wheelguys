...
def sign_detection_process(frame_queue, shared_data, running_event):
    ...
    while running_event.is_set():
        if not frame_queue.empty():
            ...
            if final_detected:
                shared_data['latest_sign_text'] = f"final ({final_conf*100:.1f}%)"
            else:
                valid_candidates = []
                for cls in class_names:
                    if cls == 'final':
...
def main():
    ...
    try:
        while camera.isOpened():
            ...
            if raw_sign in has_triggered and has_triggered[raw_sign]:
                raw_sign = "없음"
            elif raw_sign == "finish" and has_triggered['final']:
                raw_sign = "없음"
            elif raw_sign == "final" and has_triggered['finish']:
                raw_sign = "없음"
                    ...
                    if sign_class in intersection_signs:
                        ...
                        if is_valid_sign:
                            ...
                            if current_zone == ZONE_OUTER:
                                ...
                                # 3단계 (ZONE_EXIT): 탈출 명령 표지판이 들어왔을 때 최종 탈출 명령 주입 후 즉시 복귀 단계 진입 (Rising Edge)
                                if sign_class == "left":
                                    current_command = 1
                                elif sign_class == "right":
                                    current_command = 2
                                ...
                                print(f"\n[FSM] ZONE_EXIT -> ZONE_RECOVERY: {sign_class} 감지 (5초 뒤 외곽 복귀 예약, cmd: {current_command})")
                    else:
                        ...
                        has_triggered[sign_class] = True
                        if sign_class in ["finish", "final"]:
                            has_triggered["finish"] = True
                            has_triggered["final"] = True
                        ...
                        if sign_class == "stop":
                            ...
                            final_time_end = current_time + 5.5
            ...
            input_cmd_data[0, current_command] = 1
